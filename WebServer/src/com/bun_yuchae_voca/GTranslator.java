...
public class GTranslator {
   ...
   * @param out print stream
   */
  public  void translateTextWithOptionsAndModel(
      String sourceText,
      String sourceLang,
      String targetLang,
      PrintStream out) {
  ...
  public  String translateTextWithOptions(
    ...
    TranslateOption srcLang = TranslateOption.sourceLanguage(sourceLang);
    TranslateOption tgtLang = TranslateOption.targetLanguage(targetLang);
    TranslateOption eng = TranslateOption.targetLanguage(targetLang);
    
    if(sourceLang!=com.bun_yuchae_voca.Language.Google.English){
    	Translation firstTranslation = translate.translate(sourceText, srcLang, eng);
    	String first = firstTranslation.getTranslatedText();
    	Translation secondTranslation = translate.translate(first, eng, tgtLang);
    	return secondTranslation.getTranslatedText();
    }
    else if(sourceLang.compareTo(targetLang)==0){
    	return sourceText;
    }
    else{
  ...
  public Translate createTranslateService() {	  
    return TranslateOptions.newBuilder().setApiKey(KeyManager.getInstance().requestKey(CompanyType.Google)).build().getService();
  }  
}